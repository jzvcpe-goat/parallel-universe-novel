...
1. Do not fabricate Data API readiness, Supabase health, service ids, origins or
   publishable/anon client keys in docs, fixtures, CI artifacts or GitHub
   variables.
2. The correct next operator path is still P147 -> P168:
   fill the ignored local Data API evidence, run P156 local Data API boundary
   guard, compile the runtime assignment, run `remote-health:check`, then run
   `prepare:edge-only-data-api-strict-intake`.
3. Supabase publishable keys and legacy anon keys are browser-allowed client
   credentials, not private secrets. The safety boundary is RLS and least
   privilege grants. They still stay in ignored local files so project-specific
   configuration does not drift into logs, evidence artifacts or GitHub
   variables.
4. P156 must also reject `secret` / `service_role` keys, writer passwords and
   provider keys, and must prove the intent env, `.env.local` and
   `.env.local.sync` point at the same Supabase project before strict health
   evidence is accepted.
5. P150/P151/P145 are allowed to pass in waiting mode in normal CI; strict mode
   is only valid after the operator supplies real external evidence.
6. If this blocker reappears after a new commit, first run
...
but metadata presence is still weaker than content proof. P172 upgrades this
operator handoff packet to the same pattern used by the neighboring P131/P133
operator artifacts: download the artifact from the current Pages run, validate
the JSON, and write a small attestation artifact.

Engineering rule:

1. A handoff artifact that affects launch confidence should not stop at
   "uploaded and non-empty" once it is part of the operator chain.
2. P170 remains the generator for the operations-continuity packet. P171 owns
   current-run upload coverage. P172 owns downloaded content attestation.
3. The content check must preserve the launch blocker: `nextGoal` remains
   `operator-assignment-evidence-intake`, `valuesIncluded` is `false`, and the
   boundary flags must still deny service creation, secret upload, env writes,
   live promotion and evidence completion.
4. This is evidence-chain hardening only; it does not touch Data API resources
   or runtime behavior.
...
1. Every root-test gate that becomes part of the operator handoff should either
   ...
   evidence.
2. `operator-operations-continuity` is generated by P170, uploaded by P171, and
...
3. The fast path reuses existing gates: P156 local Data API boundary guard, P140 intent
...
1. The selected `operator-assignment-evidence-intake` command chain is now 10
   commands: bootstrap intent env, P156 local Data API boundary guard, P150 readiness
...
## 2026-06-21 P157 Local Data API Boundary Guard Handoff Propagation

P156 made the local Data API boundary guard part of root `npm run test`, but the
...
1. The selected `operator-assignment-evidence-intake` command chain is now 10
   commands after P159: bootstrap intent env, P156 local Data API boundary guard, P150
...
3. P147 `nextCommand` points at the P156 local Data API boundary guard after bootstrap;
...
1. P156 的历史 gate 名称保留为 `Local Secret Guard`，但它不能把 Supabase
   publishable key 或 legacy anon key 当作私密 secret。它们是浏览器允许使用的
   client credential，真正边界是 RLS 和最小权限 grants。
2. P156 的实际语义是：必须能找到 publishable/anon key；必须禁止
   `secret`/`service_role` key、writer password、database URL、provider key 等真实
   secret；不能把完整 key 打进日志或 evidence artifact；`.env.local.sync` 不能进入
   Git history。
...
3. `.env.local` / `.env.local.sync` 只允许保存本地 health check 所需的 Supabase
   URL 和 publishable / legacy anon key 名：`VITE_SUPABASE_URL`、`SUPABASE_URL`、
...
5. P156 必须检查 intent env、`.env.local`、`.env.local.sync` 是否指向同一个 Supabase
   project；如果多个本地 key 存在，只能输出短 hash 指纹核对一致性，不能输出完整 key。
6. P156 artifact 只包含 booleans、counts、短指纹和一致性状态，不能包含完整 URL、
   service id、key、password、provider output、prompt、reference material 或
   candidate prose。
7. root `npm run test` 的顺序固定为 P149 -> P156 -> P150 -> P146，避免文档和机器 gate
...
3. P130/P131 的 operator handoff command count 在 P159 后是 10：bootstrap intent env、
   P156 local Data API boundary guard、P150 readiness diagnosis、prepare intent、compile public
...
3. P120/P121/P122/P123 这段证据链必须顺序执行；并行跑 P120/P122 会故意制造
   stale reference。
4. `prepare:loop-next-goal-local` 成功后应直接跑 P123。如果又手动跑了一次 P121，
...
2. 新增 `prepare:loop-next-goal-local`，本地按顺序刷新 P4/privacy/runtime image、
   ignored assignment draft、P113/P119/P120/P107/P121/P122。
3. 该命令可能刷新 `deploy/runtime-production/remote-assignment.local.json`，所以不能
   进入 root `npm run test` 或普通 CI。
4. 如果这个 helper 之后 P121 或 P122 仍失败，才说明下一目标证据链真的不一致。
5. 如果 helper 成功后又手动跑了一次 P121，后面必须重跑 P122，再跑 P123；否则
   P123 会因为 P122 引用旧 P121 而失败。
...
4. `.env.local.sync` 是 writer 密码和 Supabase publishable/anon 配置的单点故障；
   publishable/anon key 本身不是私密 secret，但它仍必须离开 Git/Vercel 以避免环境
   混淆和 artifact 泄漏；writer password 则必须进入可信密码管理器或加密备份。
...
## 2026-06-20 P123 Edge-Only Blocker Projection

P138 让 edge-only 成为当前上线拓扑后，P123 不能再把 legacy
`remote-assignment.local.json` 的 full-remote blocking stages 原样展示给运营侧。
否则 handoff 会一边说“远端 Agent 不需要”，一边要求补
`assignment-agent-service-id`、`assignment-agent-origin` 或 agent health，团队很容易
为了过 gate 填假服务。

本轮原则：

1. P123 public handoff 的 blocking stages 来自
   `runtime-assignment.intent.local.json` 和 remote health evidence 的公开投影。
2. edge-only 下允许显示的真实阻塞项是前端、托管数据 API、RLS/publishable
   配置和 `health_probe` 健康证据。
3. edge-only 下禁止把 legacy Agent service id、origin、secret-store confirmation
   或 health 作为当前 blocker。
4. P124 必须下载/读取 P123 artifact 并断言不含 `assignment-agent-*` 或
