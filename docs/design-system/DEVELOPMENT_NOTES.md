...
用户明确要求 P4 从头做，并废弃此前围绕“西幻/非游戏化/禁古代官署职业”形成的临时约束逻辑。因此本轮再补一层防回归：

1. 人工规则文档里的现代类 ID 统一对齐运行时 registry：`modern-other / kernel-modern-other`。
2. 后端删除未命中文档 profile 时的科幻兜底猜测；没有 active profile 就继续通过苏格拉底式对话澄清，不生成 off-registry 约束。
3. 后端移除旧样本中“地下城”等词作为 tone hardcode；如果未来要支持此方向，必须先进入 `genre-runtime-rules.v1.json` 的文档化 profile。
4. `scan-p4-rule-source.mjs` 现在同时检查 stale doc id 与退役 prompt-case 词，不允许它们重新进入 agent/backend runtime 或 P4 tests。
