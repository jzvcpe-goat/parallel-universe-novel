...
这次按最新产品判断重新收敛 P4：此前围绕某组一次性测试断点形成的约束逻辑整体废弃，P4 只承认文档里的 `ConstraintProfile + GenreKernel`。如果用户选择某个题材，运行时只应激活对应文档 profile；如果文档没有该 profile，就不能临时把一个测试断点写成硬规则。
...
3. 旧的一次性 P4 词项只保留在 `scan-p4-rule-source.mjs` 的 retired list 里，作为禁止回流的检查输入。
