...
- The `operator-evidence-return-fast-path` artifact is generated by P168 before
  upload and proves the operator return command contract is retained without
  requiring external Data API values.
...
30. Every Pages run must upload `operator-evidence-return-fast-path` so the
    release owner can verify the operator return command contract is retained as
    current-run evidence without exposing local Data API values.
