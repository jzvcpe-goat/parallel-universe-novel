...
把 GitHub Actions 的上线证据从“日志里看见通过”升级成可自动核验的 artifact gate。当前 Pages workflow 必须留下三十类可下载证据：
...
21. `Upload operator assignment transition fixture`
22. `Upload runtime image local smoke`
23. `Upload zero-cost reader edge sync`
24. `Upload remote health evidence`
25. `Upload reference privacy evidence`
26. `Upload public projection privacy evidence`
27. `Upload reference work encryption completion evidence`
28. `Upload representative work custody evidence`
29. `Upload kernel constraint reference encryption evidence`
30. `Upload artifact`
