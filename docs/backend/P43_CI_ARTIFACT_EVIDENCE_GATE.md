...
把 GitHub Actions 的上线证据从“日志里看见通过”升级成可自动核验的 artifact gate。当前 Pages workflow 必须留下二十七类可下载证据：
...
21. `Upload remote health evidence`
22. `Upload reference privacy evidence`
23. `Upload public projection privacy evidence`
24. `Upload reference work encryption completion evidence`
25. `Upload representative work custody evidence`
26. `Upload kernel constraint reference encryption evidence`
27. `Upload artifact`
