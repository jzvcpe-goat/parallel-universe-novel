...
把 GitHub Actions 的上线证据从“日志里看见通过”升级成可自动核验的 artifact gate。当前 Pages workflow 必须留下三十七类可下载证据：
...
`representative-work-custody`、`kernel-constraint-reference-encryption` 和
`kernel-constraint-legal-privacy-loop`
...
`operator-evidence-return-fast-path` 由 P168 在上传前生成，并由 P174
`check:operator-evidence-return-fast-path-artifact` 再下载核验，
`operator-operations-continuity` 由 P170 生成，并由 P172
`check:operator-operations-continuity-artifact` 再下载核验，
...
`narrative-okf-knowledge-layer`、`narrative-okf-runtime-consumption` 和
`okf-runtime-image-context` 的 JSON 内容由
P176_NARRATIVE_OKF_RELEASE_ARTIFACT_ATTESTATION 再下载核验：
...
- `narrative-okf-knowledge-layer` exists and is non-empty.
- `narrative-okf-runtime-consumption` exists and is non-empty.
- `okf-runtime-image-context` exists and is non-empty.
...
- `kernel-constraint-legal-privacy-loop` exists and is non-empty.
...
27. `Upload Narrative OKF knowledge layer evidence`
28. `Upload Narrative OKF runtime consumption evidence`
29. `Upload OKF runtime image context evidence`
30. `Upload reference privacy evidence`
31. `Upload public projection privacy evidence`
32. `Upload reference work encryption completion evidence`
33. `Upload representative work custody evidence`
34. `Upload kernel constraint reference encryption evidence`
35. `Upload kernel constraint legal privacy loop evidence`
33. `Upload artifact`
