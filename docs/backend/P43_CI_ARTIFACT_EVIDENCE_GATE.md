...
把 GitHub Actions 的上线证据从“日志里看见通过”升级成可自动核验的 artifact gate。当前 Pages workflow 必须留下二十三类可下载证据：
...
18. `Upload runtime image local smoke`
19. `Upload reference privacy evidence`
20. `Upload public projection privacy evidence`
21. `Upload reference work encryption completion evidence`
22. `Upload representative work custody evidence`
23. `Upload artifact`
...
14. Pages workflow runs P115 after P132 so `runtime-image-local-smoke` content
   is validated separately from artifact metadata.
13. Root `npm run test` runs P107 so the full artifact set always has an
