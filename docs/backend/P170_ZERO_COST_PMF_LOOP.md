...
- The creator app runs on the author's computer in local mode.
...
4. The local creator app syncs the request queue from the author's computer.
...
The production schema contains only product tables, constraints, RLS, triggers,
and baseline feature flags. Demo works are not seeded by the production schema.
For local development only, use `deploy/supabase/seeds/demo_works.sql`.
...
creator allowlist table, normalizes legacy creator-client app mode values to
`local`, and replaces the related RLS policies. It does not seed demo works,
chapters, or reader requests.
...
work creation, branch/chapter writes, request status mutation,
...
`creator_authorizations` before `role=creator` can be written. Creator client
heartbeats must use `app_mode = 'local'` and an existing creator profile. The
...
The full P0 live proof requires one non-anonymous and allowlisted creator
...
chapter, writes `publish_events`, updates the request to `published`, and
