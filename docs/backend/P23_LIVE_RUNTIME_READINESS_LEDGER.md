...
P40 扩展：账本不再只证明 `/health` 在线，还必须记录 `POST /v1/workflows/socratic-create` 的 direct workflow preflight。预检只保存公开响应摘要：`responseMode`、`candidateDraft.status`、候选正文长度、追问数量和内部字段泄漏结果，不保存候选正文、原始 Tool Bridge payload 或模型内部信息。
...
- Agent Runtime `POST /v1/workflows/socratic-create` 返回 `responseMode=public`
- 该 workflow preflight 返回 `candidateDraft.status=candidate`
- 候选正文长度为 300-900 compact characters
- 追问数量为 0-2
- 预检响应不包含 `runtimeArtifact`、`sourceRefs`、`kernelId`、`profileId`、`activeConstraints`、`activeKernels`、`sourceLabels`、`runTrace`、`ledger` 或 `cost`
...
- `package.json` 暴露 `check:runtime-readiness-ledger`。
- 根目录 `npm run test` 先生成账本，再立即验证账本结构、状态枚举、阻塞项和隐私边界。
- GitHub Actions 中的 readiness ledger 必须通过 `gh variable list` 或 `${{ vars.* }}` workflow context 读取 GitHub repository variables，并记录 `repoVariables.checked=true` 与 `repoVariables.source`。
...
- Readiness ledger 必须包含 `creator-workflow-preflight` 检查，证明 `POST /v1/workflows/socratic-create` 能返回 public candidate。
