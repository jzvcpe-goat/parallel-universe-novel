...
| --- | --- | --- | --- | --- |
| `narrative-runtime-engine` | Narrative Runtime Engine | `partial` | `RuntimeArtifact` covers constraint set, kernel selection, scene plan, state preview, time consistency, quality brake, branch result; Reader choices now carry route trace and WorldInstance patch candidates; Reader branch publish candidate consumes TimeEngine candidate events; P59 proves a database transaction rollback fixture; P60 proves branch publish operator authorization as candidate ledger; P61 proves branch commit draft with multi-table rollback fixture; P62 writes a private production branch table row and audit event; P63 writes a Reader-visible `public_branch_releases` row and audit event; P64 writes a `time_engine_telemetry_fits` row; P98 writes confirmed Studio canon promotion into `production_canon_commits` plus `analytics_events` with `production_canon_promotion`. | Remote live runtime trace is not yet proven. |
...
- The matrix classifies all sixteen Pages artifacts into
...
P109 keeps the GitHub repository variable layer aligned with the public runtime
boundary:

- Contract: `P109_GITHUB_RUNTIME_VARIABLE_BOUNDARY_GUARD`.
- `check:github-runtime-variable-boundary` is part of root `npm run test`.
- The gate reads repository variables with `gh variable list` when available.
- Allowed variables are limited to public runtime origins, remote service ids
  and non-secret secret-store attestation flags.
- The generated `github-runtime-variable-boundary` artifact contains names and
  issue codes only, never variable values.

This does not provision remote services. It prevents database URLs, Tool Bridge
token values, model keys, private keys or provider API tokens from leaking into
GitHub repository variables or CI artifacts.

## P110 Runtime Placeholder Sentinel Guard

P110 prevents P87/P75/P79/P109 placeholder drift:

- Contract: `P110_RUNTIME_PLACEHOLDER_SENTINEL_GUARD`.
- `check:runtime-placeholder-sentinel` is part of root `npm run test`.
- The gate builds a temporary `FILL_*` assignment fixture under
  `artifacts/runtime/`.
- P75 must return `remote_assignment_incomplete`.
- P79 must return `assignment_execution_incomplete`.
- P109 must recognize the same placeholder sentinels for GitHub repository
  variables.

This does not fill the assignment. It prevents `FILL_API_SERVICE_ID`,
`https://FILL_API_HOST`, `REPLACE_ME`, `YOUR_*`, `TODO_*` and `<...>` template
values from being mistaken for production evidence.

## P116 Remote Assignment Env Apply Gate

P116 makes the operator assignment intake safer without changing the blocker
taxonomy:

- Contract: `P116_REMOTE_ASSIGNMENT_ENV_APPLY_GATE`.
- `apply:remote-assignment-env` writes only the ignored
  `deploy/runtime-production/remote-assignment.local.json` after explicit
  confirmation.
- `check:remote-assignment-env-apply` is part of root `npm run test` and never
  writes the local assignment.
- The `remote-assignment-env-apply` artifact is redacted and must not expose
  service ids, origins, secrets, provider prompt plumbing, reference-work
  material, `sourceRefs`, `profile.id` or `kernel.id`.

This does not make remote runtime ready. It only converts non-secret operator
environment values into the same P75 assignment file, then preserves the P75,
P73, P66, P23 and P76 strict gates.

## P117 Remote Assignment Env Dry-Run Gate

P117 validates the same non-secret operator environment values before P116 writes
ignored local state:

- Contract: `P117_REMOTE_ASSIGNMENT_ENV_DRY_RUN_GATE`.
- `check:remote-assignment-env-dry-run` is part of root `npm run test`.
- With no operator env, it passes as `operator_env_not_supplied` for CI.
- With operator env, it rejects partial values, placeholders, invalid origins
  and secret-looking material before any write.
- The `remote-assignment-env-dry-run` artifact is redacted and must not expose
  service ids, origins, secrets, provider prompt plumbing, reference-work
  material, `sourceRefs`, `profile.id` or `kernel.id`.

This does not make remote runtime ready. It only proves the P116 apply inputs
are safe enough to attempt the ignored local assignment write.

## P52 Runtime Completion Matrix Refresh

P52 refreshed this matrix after P49 and P51, and P98 updates the canon promotion proof:
...
- P98 Canon Promotion Transaction Gate proves confirmed Studio canon promotion with `production_canon_promotion`, `production_canon_commits`, `analytics_events`, and a pre-commit rollback fixture.
...
P103 makes the learned promotion workflow auditable without activating learned tracks as production gates:

- Contract: `P103_LEARNED_EVAL_PROMOTION_WORKFLOW_GATE`.
- `backend/src/narrativeos/eval/learned_promotion_workflow.py` and `learned_reranker_promotion_workflow.py` define evaluator and reranker promotion workflows.
- Both tracks use `manual_approval`, review records with `asset_type = learned_promotion`, and explicit approve/revoke states.
- `/v1/ops/learned-promotion`, `/v1/ops/learned-reranker-promotion`, `/v1/ops/learned-rollout/{track}/activate`, and `/v1/ops/learned-rollout/{track}/rollback` are the Ops surfaces for evidence, approval, activation and rollback.
- `check:learned-eval-promotion-workflow` is required by root `npm run test`.
...
- `/v1/canon/commit` stores the same trace in the compatibility `canon_ledger_only` record and writes the confirmed promotion to `production_canon_commits`.
...
Remaining gaps stay explicit: P56 is not remote live runtime and not production
operator authorization. P98 now proves the local durable multi-table canon
promotion path.
...
`scripts/check-remote-assignment-fill-plan-artifact.mjs` downloads or reads the
P105 `remote-assignment-fill-plan` artifact and validates its JSON plus
Markdown content:

Command: `npm run check:remote-assignment-fill-plan-artifact`.

- artifact gate, repository and head sha,
- current-head API and Agent Runtime image refs in release/CI mode,
- source workspace no-git mode keeps runtime image and handoff content blockers
  visible,
- six required fill areas and strict validation sequence,
- blocker preservation for `remote-assignment-file-present` and
  `activation-control`,
- no secrets, reference work names, `sourceRefs`, `profile.id`, `kernel.id`,
  raw state or provider prompt plumbing.

Current decision is that P106 closes the artifact-content gap left by P105. It
does not make remote runtime ready; it proves the fill plan itself is safe and
current.

## P118 Remote Assignment Strict-Run Package

`scripts/check-remote-assignment-strict-run-package.mjs` turns the P105/P106
fill plan plus P116/P117 no-write/apply evidence and the current blocker ledger
into one ordered operator execution package:

Command: `npm run check:remote-assignment-strict-run-package`.

- Contract: `P118_REMOTE_ASSIGNMENT_STRICT_RUN_PACKAGE`.
- The package covers P117/P116/P75/P79/P73/P66/P23/P65/P76/P78/P85/P96.
- It emits JSON and Markdown artifacts named
  `remote-assignment-strict-run-package`.
- `scripts/check-remote-assignment-strict-run-package-artifact.mjs` validates
  local or current-run artifact content with
...
Current decision is that P118 is an operator strict-run package gate. It
...
`scripts/check-remote-operator-return-intake.mjs` validates the evidence that
comes back after the P119 handoff: P75 assignment intake, P117 env dry-run, P113
image drift, P85 blocker ledger, P78 activation control and P23 live readiness.

Command: `npm run check:remote-operator-return-intake`.

- Contract: `P120_REMOTE_OPERATOR_RETURN_INTAKE`.
- It emits JSON and Markdown artifacts named `remote-operator-return-intake`.
- `scripts/check-remote-operator-return-intake-artifact.mjs` validates local or
  current-run return-intake content with
  `CHECK_REMOTE_OPERATOR_RETURN_INTAKE_ARTIFACT_REQUIRED=true`.
- Boundary flags prove it does not write local assignment state, create
  services, set GitHub variables, store provider secrets, promote live runtime
  or treat fixtures as production readiness.

Current decision is that P120 is an operator return-state gate. It reduces
handoff-return ambiguity but intentionally preserves remote runtime blockers
until assignment, health, live trace, cutover and activation gates pass.
...
P97 brings the existing backend provider routing stack into the root release
gate:

- `ProviderRoutingService` selects candidate and renderer tracks without
  hardcoding a public provider dependency.
- `BudgetedLLMBackend` proves budget-aware blocking and safe fallback.
- Provider rollout rollback can disable a candidate backend without changing
  public Creator or Reader code.
- Runtime receipts keep provider, cache, budget, latency and fallback evidence
  inside backend/Ops surfaces.
- Public Creator projection removes `model_status` and `harness_trace`; public
  UI scans keep provider/cost/debug terms out of Reader and Creator surfaces.

Current decision is that P97 proves local cost-aware routing and public
projection separation. It does not clear P85 remote runtime blockers or prove
remote public provider smoke.
