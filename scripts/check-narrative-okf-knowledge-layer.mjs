...
assert(
  String(packageJson.scripts.test || '').includes('npm run check:runtime-rule-handshake && npm run check:narrative-okf-knowledge-layer && npm run check:runtime-artifact-contract'),
  'root npm run test must run P165 between runtime rule handshake and runtime artifact contract',
)
