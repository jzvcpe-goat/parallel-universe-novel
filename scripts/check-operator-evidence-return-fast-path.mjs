...
assert(
  rootTest.includes('npm run check:ci-artifact-content-coverage && npm run check:current-head-operator-evidence-refresh && npm run check:operator-evidence-return-fast-path && npm run check:loop-next-goal-ledger'),
  'root npm run test must run the lightweight P168 contract check between P164 and P121',
)
