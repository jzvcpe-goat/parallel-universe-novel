...
const strictRunPackage = [
  ...
  stage('env-dry-run', 'Validate operator env file without writes', 'REMOTE_ASSIGNMENT_ENV_FILE=deploy/runtime-production/remote-assignment.env.local REQUIRE_REMOTE_ASSIGNMENT_ENV_DRY_RUN_READY=true npm run check:remote-assignment-env-dry-run', 'P117', 'deployment operator', 'all REMOTE_* fields are supplied outside Git'),
  stage('env-apply', 'Materialize ignored assignment from operator env file', 'REMOTE_ASSIGNMENT_ENV_FILE=deploy/runtime-production/remote-assignment.env.local REMOTE_ASSIGNMENT_ENV_APPLY_CONFIRM=true npm run apply:remote-assignment-env', 'P116', 'deployment operator', 'P117 is ready for apply'),
