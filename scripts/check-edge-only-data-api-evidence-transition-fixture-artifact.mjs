...
function validatePacket(payload, expectedHeadSha) {
  ...
  if (payload.transition?.currentDataApiEvidenceAlreadyReady === true) {
    assert(payload.transition?.strictHealthReady === true, 'P148 current-ready path must prove strict health readiness')
    assert(payload.transition?.expectedNextGoalWithRealEvidence === 'strict-live-activation-proof', 'P148 current-ready next goal mismatch')
    assert(payload.sourceEvidence?.currentHealthAttestation?.gate === 'P145_REMOTE_HEALTH_EVIDENCE_ARTIFACT_GATE', 'P148 current-ready path must cite P145')
    assert(payload.sourceEvidence?.currentHealthAttestation?.status === 'passed', 'P148 current-ready P145 status mismatch')
    assert(payload.sourceEvidence?.currentHealthAttestation?.healthReady === true, 'P148 current-ready P145 healthReady mismatch')
    assert(payload.sourceEvidence?.currentAssignmentIntake?.gate === 'P75_REMOTE_RUNTIME_ASSIGNMENT_INTAKE', 'P148 current-ready path must cite P75')
    assert(payload.sourceEvidence?.currentAssignmentIntake?.decision === 'remote_assignment_ready', 'P148 current-ready P75 decision mismatch')
    assert(Array.isArray(payload.sourceEvidence?.currentAssignmentIntake?.blockedStages), 'P148 current-ready P75 blockedStages must be present')
    assert(payload.sourceEvidence.currentAssignmentIntake.blockedStages.length === 0, 'P148 current-ready P75 must have no blockers')
    assert(payload.boundary?.fixtureOnly === true, 'P148 must remain fixture-only')
    ...
    assert(payload.boundary?.leavesRealHealthReadyArtifactAsCurrentState === true, 'P148 current-ready path must preserve real health evidence')
